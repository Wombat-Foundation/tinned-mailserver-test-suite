...
| SENDER_ADDRESS_MAIN | The email address of the local user to receive the test mail |
| RECIPIENT_ADDRESS   | The external email address sending the test mail             |
| PAYLOAD_PATH        | Path to the EICAR.COM-ZIP file                               |

### Description

This test email contains the EICAR test signature inside a ZIP file. It is sent as an inbound message to the mailserver under test.

- connects via SMTP port (port 25)
- uses STARTTLS
- sends email with EICAR.COM-ZIP.zip attachment

### Pass criteria

This test is considered passed when the email is **successfully rejected** (Swaks exit code 24).

## 204 inbound_eicar_com
...
| SENDER_ADDRESS_MAIN | The email address of the local user to receive the test mail |
| RECIPIENT_ADDRESS   | The external email address sending the test mail             |
| PAYLOAD_PATH        | Path to the EICAR.COM file                                   |

### Description

This test email contains the EICAR test signature as a raw .COM file attachment. It is sent as an inbound message to the mailserver under test.

- connects via SMTP port (port 25)
- uses STARTTLS
- sends email with EICAR.COM attachment

### Pass criteria

This test is considered passed when the email is **successfully rejected** (Swaks exit code 24).

## 205 inbound_eicar_com2_zip
...
| SENDER_ADDRESS_MAIN | The email address of the local user to receive the test mail |
| RECIPIENT_ADDRESS   | The external email address sending the test mail             |
| PAYLOAD_PATH        | Path to the EICAR.COM2-ZIP file                              |

### Description

This test email contains the EICAR test signature inside a nested ZIP file (double compressed). It is sent as an inbound message to the mailserver under test.

- connects via SMTP port (port 25)
- uses STARTTLS
- sends email with EICAR.COM2-ZIP.zip attachment
